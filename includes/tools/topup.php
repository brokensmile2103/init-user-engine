...
function init_plugin_suite_user_engine_render_topup_page() {
    ...
    if (
        ...
        if ( 0 === $amount || empty( $user_ids ) || ! in_array( $type, [ 'coin', 'cash' ], true ) ) {
            ...
            // =========================
            // Gom inbox theo batch
            // Cấu trúc: $inbox_batches[ 'coin'|'cash' ][ 'add'|'deduct' ][ $applied ] = [ user_id, ... ]
            // =========================
            $inbox_batches = [
                'coin' => [ 'add' => [], 'deduct' => [] ],
                'cash' => [ 'add' => [], 'deduct' => [] ],
            ...
            foreach ( $user_ids as $user_id ) {
                if ( 'coin' === $type ) {
                    ...
                    if ( 0 !== $delta ) {
                        ...
                        // === Không gửi ngay từng user nữa, mà gom vào batch ===
                        if ( $applied > 0 ) {
            ...
            // =========================
            // Helper gửi batch cho 1 loại tiền tệ (coin/cash)
            $send_bulk_for_type = function( $money_type, $label ) use ( $inbox_batches ) {
                // add và deduct tách riêng vì title/content khác nhau
                foreach ( [ 'add', 'deduct' ] as $chg ) {
                    if ( empty( $inbox_batches[ $money_type ][ $chg ] ) ) {
                        continue;
                    }
                    foreach ( $inbox_batches[ $money_type ][ $chg ] as $applied => $uids ) {
                        ...
                        // Gửi BULK một phát cho nhóm có cùng change & applied
                        init_plugin_suite_user_engine_send_inbox_to_users_bulk(
                            ...
                            '',
                            0,
                            // Bro có thể filter chunk size qua 'init_user_engine_inbox_bulk_chunk_size'
            ...
            // Gửi theo từng loại (coin/cash) đã gom
